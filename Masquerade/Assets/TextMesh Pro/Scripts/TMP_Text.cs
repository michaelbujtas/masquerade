    ...
    public class TMP_Text : MaskableGraphic
    {
        ...
        public void SetCharArray(char[] sourceText)
        {
            ...
            for (int i = 0; i < sourceText.Length; i++)
            {
                if (sourceText[i] == 92 && i < sourceText.Length - 1)
                {
                    ...
                    {
                        case 110: // \n LineFeed
                            if (writeIndex + 1 > m_char_buffer.Length)
                                Array.Resize(ref m_char_buffer, m_char_buffer.Length * 2);
        ...
        protected void SetTextArrayToCharArray(char[] sourceText, ref int[] charBuffer)
        {
            ...
            for (int i = 0; i < m_charArray_Length; i++)
            {
                ...
                {
                    if (writeIndex + 1 > charBuffer.Length)
                        Array.Resize(ref charBuffer, charBuffer.Length * 2);
